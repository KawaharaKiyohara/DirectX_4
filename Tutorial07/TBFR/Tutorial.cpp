...
/*!
*@brief
*  ZPrepass用のスキンモデルエフェクト。
*/
class CZprepassSkinModelEffect : public CModelEffect {
public:
	CZprepassSkinModelEffect()
	{
		m_vsShader.Load("Assets/shader/model.fx", "VSMainSkinZPrepass", CShader::EnType::VS);
...
/*!
*@brief
*  ZPrepass用のスキンなしモデルエフェクト。
*/
class CZprepassNonSkinModelEffect : public CModelEffect {
public:
	CZprepassNonSkinModelEffect()
	{
		m_vsShader.Load("Assets/shader/model.fx", "VSMainZPrepass", CShader::EnType::VS);
		m_psShader.Load("Assets/shader/model.fx", "PSMain", CShader::EnType::PS);
		isSkining = false;
	}
};
class ForwardRender : public IGameObject {
	...
	CShader csTBFR;										//TBFR(Tile Based Forward Rendering)用のコンピュートシェーダー。
	SLight m_light;										//ライト。
...
public:
	...
	bool Start() override
	{
		...
		//TBFR用のコンピュートシェーダー。
		csTBFR.Load("Assets/shader/TBFR.fx", "CSMain", CShader::EnType::CS);
	...
	*   深度値を書き込むためのパス。
	...
	void ZPrepass(CRenderContext& rc)
	{
		...
		bgModel.Draw(rc, camera.GetViewMatrix(), camera.GetProjectionMatrix(), true);
		rc.OMSetRenderTargets(1, NULL);
	}
	/*!------------------------------------------------------------------
	* @brief	タイルごとの動的光源のリストを作成。
	------------------------------------------------------------------*/
	void CreateLightingList(CRenderContext& rc)
	{
		//ポイントライトのリストを設定。
		rc.UpdateSubresource(m_pointLightSB, m_pointLightList);

		rc.CSSetShader(csTBFR);
		//深度テクスチャを設定。
		rc.CSSetShaderResource(0, depthBuffer.GetRenderTargetSRV());
		//ポイントライトのリストを設定。
		rc.CSSetShaderResource(1, m_pointLightSRV);
		//出力先を設定。
		rc.CSSetUnorderedAccessView(0, m_pointLightListInTileUAV);
		//カメラ定数バッファの設定。
	...
	* @brief	シーンの描画。
	...
	void RenderScene(CRenderContext& rc)
	{
		...
		//ポイントライトのタイルごとのリストをt1に設定。
		rc.PSSetShaderResource(1, m_pointLightListInTileSRV);
		//ポイントライトのリストをt2に設定。
		rc.PSSetShaderResource(2, m_pointLightSRV);
